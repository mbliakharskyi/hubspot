...
const users: usersConnector.HubspotUser[] = Array.from({ length: 5 }, (_, i) => ({
  id: `id-${i}`,
  username: `username-${i}`,
  email: `username-${i}@foo.bar`,
}));

const setup = createInngestFunctionMock(syncUsersPage, 'hubspot/users.sync_page.requested');
...
describe('sync-users', () => {
  ...
  test('should continue the sync when there is a next page', async () => {
    ...
    expect(step.sendEvent).toBeCalledWith('sync-users-page', {
      name: 'hubspot/users.sync_page.requested',
      data: {
        organisationId: organisation.id,
        isFirstSync: false,
        syncStartedAt,
