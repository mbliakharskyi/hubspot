...
import { HubspotError } from '@/connectors/commons/error';
 ...
 * The middleware checks for specific errors (instances of HubspotError) that indicate a rate limit has been reached,
 ...
 * - Handles HubspotError, specifically looking for a 'Retry-After' header in the error response.
            ...
            return {
              ...
              result: {
                ...
                error: new RetryAfterError(
                  `Hubspot rate limit reached by '${fn.name}'`,
                  retryAfter,
                  {
                    cause: error,
                  }
